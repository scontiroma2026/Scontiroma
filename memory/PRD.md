...
- **[2026-08-28]** Offerta Mese Prossimo (ciclo mensile completo):
  ...
  - **Admin**: tab "Prossimo Mese" (`AdminNextMonth.jsx`): tutti i negozi con stato (Approvata/In revisione/Rifiutata/Non caricata), anteprima offerta, Approva/Rifiuta (con email al merchant), summary chips, pulsanti "Invia promemoria ora", "Esegui passaggio mese", override finestra (apri/chiudi/automatico). Endpoint `GET /api/admin/next-offers`, `POST .../{id}/approve|reject`, `.../window-override`, `.../run-rollover`, `.../run-reminders`.
  - Banner merchant "Offerta scaduta a fine mese" per stato `expired`.
  - **Test e2e curl**: POST 423 a finestra chiusa ✓, upload pending ✓, approve/reject/re-approve ✓, reminders (18 inviati) ✓, rollover simulato con backup/restore (promozione + scadenza + archivio ✓). Screenshot merchant tab + admin tab OK. Dati demo ripristinati; resta l'offerta di settembre della Trattoria (approvata) come demo reale.
