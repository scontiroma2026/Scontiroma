...
export default function DiscountDetail() {
  ...
  if (loading || !discount) {
    return <div className="mx-auto max-w-7xl px-6 py-16 text-espresso/60">Caricamento…</div>;
  }
  ...
  return (
    <main data-testid="discount-detail-page" className="mx-auto max-w-6xl px-6 py-10">
      <button onClick={() => nav(-1)} className="mb-6 flex items-center gap-2 text-sm text-espresso/70 hover:text-terracotta">
      ...
      <div className="grid gap-10 md:grid-cols-2">
        ...
        <div>
          <div className="mb-2 flex items-center gap-3 text-xs uppercase tracking-wider">
            ...
            <span className="flex items-center gap-1 text-espresso/70"><MapPin size={12} /> {m.zone}</span>
          </div>
          <h1 className="font-serif text-4xl leading-tight text-espresso">{discount.title}</h1>
          <div className="mt-2 text-lg text-espresso/70">{m.shop_name}</div>
          <p className="mt-6 text-espresso/80">{discount.description}</p>

          <Card className="mt-6 border-warm bg-parchment p-6">
            <div className="flex items-end justify-between">
              <div>
                ...
                <div className="mt-1 flex items-baseline gap-3">
                  ...
                  <span className="text-lg text-espresso/50 line-through">€{discount.original_price.toFixed(2)}</span>
                </div>
                <div className="mt-1 text-sm text-espresso/70">Risparmi <strong>€{savings}</strong></div>
              </div>
          ...
          {discount.terms && (
            <div className="mt-6 flex gap-3 rounded-lg border border-warm bg-white p-4 text-sm text-espresso/70">
          ...
          {m.address && (
            <div className="mt-4 flex items-center gap-2 text-sm text-espresso/70">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent data-testid="qr-dialog" className="max-w-sm bg-white">
          <DialogHeader>
            <DialogTitle className="font-serif text-2xl">Il tuo codice sconto</DialogTitle>
          </DialogHeader>
          {redemption && (
            <div className="flex flex-col items-center gap-4 py-4">
              <div className="rounded-xl border-2 border-terracotta/20 bg-white p-4">
              ...
              <div data-testid="redemption-code" className="text-center">
                ...
                <div className="mt-1 font-mono text-3xl font-semibold tracking-[0.3em] text-espresso">{redemption.code}</div>
              </div>
